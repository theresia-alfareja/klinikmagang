...
$insert =mysqli_query($conn, "INSERT INTO tb_obat VALUES
 (
  '".$_POST['id_obat']."',
  '".$_POST['nama_obat']."',
  '".$_POST['nama_merk']."',  
 ...
 ?>
<?php 
    $data = mysqli_query($conn, "SELECT MAX(id_obat) AS idobat FROM
    tb_obat");
    $data_akhir = mysqli_fetch_array($data);
    $id1 = $data_akhir['idobat'];
    $id2 = substr($id1,3,3); //OBT
    $id3 = $id2 + 1;
    $id4 = 'OBT'.sprintf('%03s' , $id3);
?>
<div class="w-full mt-2">
        <h1 class="text-3xl text-black pb-6 mb-1">
              <i class="fas fa-list mr-3"></i>Tambah
        </h1>
    <div>
        <?php echo $alert; ?>
    </div>           
...
<div class="mt-6">
    <div class="w-full md:flex-shrink-0 leading-loose">
       <form class="p-10 bg-white rounded shadow-xl" action="" method="post">
            <div class="">
               <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" type="text" name="id_obat" id="id_obat" value="<?php echo $id4; ?>" readonly/>
            </div>
          <p class="text-lg text-gray-800 font-medium mb-1 mt-3">Nama obat</p>
            <div class="">
               <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" type="text" name="nama_obat" id="nama_obat">
            </div>          
          <p class="text-lg text-gray-800 font-medium mb-1 mt-3">Pilih Merk</p>
            <div class="">
               <select type="text" class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" name="nama_merk" placeholder="nama_merk" required>
                    <option value="">~ Pilih ~</option>
                     <?php
                     $merk = mysqli_query($conn, "SELECT * FROM tb_merk_obat ORDER BY id_merk_obat ASC");
                     while($m = mysqli_fetch_array($merk)){
                     ?>
                  <option value="<?php echo $m['id_merk_obat'] ?>"><?php echo $m['nama_merk'] ?></option>
                  <?php } ?>
                 </select>
            </div>  
          <p class="text-lg text-gray-800 font-medium mb-1 mt-3">Satuan Obat</p>
            <div class="">
               <select type="text" class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" name="satuan_obat" required>
                <option>Tablet</option>
                <option>Kapsul</option>
                <option>Sirup</option>
               </select>
            </div>
          <p class="text-lg text-gray-800 font-medium mb-1 mt-3">Stok</p>
            <div class="">
               <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" type="text" name="stok" id="stok">
            </div>
          <p class="text-lg text-gray-800 font-medium mb-1 mt-3">Harga Beli</p>
            <div class="">
               <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" type="text" name="harga_beli" id="harga_beli">
            </div>
          <p class="text-lg text-gray-800 font-medium mb-1 mt-3">Harga jual</p>
            <div class="">
               <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" type="text" name="harga_jual" id="harga_jual">
            </div>
          <p class="text-lg text-gray-800 font-medium mb-1 mt-3">Manfaat Obat</p>
            <div class="">
               <textarea class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" name="manfaat" id="manfaat"></textarea>
            </div> 
            <br>
