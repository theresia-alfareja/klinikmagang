...
$insert =mysqli_query($conn, "INSERT INTO tb_merk_obat VALUES
 (
  '".$_POST['id_merk']."',
  '".$_POST['nama_merk']."'
)"
 ...
 ?>
<?php 
    $data = mysqli_query($conn, "SELECT MAX(id_merk_obat) AS idmer FROM
    tb_merk_obat");
    $data_akhir = mysqli_fetch_array($data);
    $id1 = $data_akhir['idmer'];
    $id2 = substr($id1,2,2); //ACT
    $id3 = $id2 + 1;
    $id4 = 'M'.sprintf('%02s' , $id3);
?>
<div class="w-full mt-2">
        <h1 class="text-3xl text-black pb-6 mb-1">
              <i class="fas fa-list mr-3"></i>Tambah Merk Obat
        </h1>          
</div>
<div class="mt-6">
    <div class="w-full md:flex-shrink-0 leading-loose">
       <form class="p-10 bg-white rounded shadow-xl" action="" method="post">
            <div class="">
               <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" type="text" name="id_merk" id="id_merk" value="<?php echo $id4; ?>" readonly/>
            </div>      
          <p class="text-lg text-gray-800 font-medium mb-1 mt-3">Nama Merk</p>
            <div class="">
               <input class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" type="text" name="nama_merk" id="nama_merk" required>
            </div>    
            <br>
